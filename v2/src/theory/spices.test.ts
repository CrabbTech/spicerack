...
describe('v2.1 spices', () => {
  it('deceptive cadence swaps the post-V tonic for vi / bVI', () => {
    const apps = SPICES['deceptive-cadence'].find(slotsOf('I', 'IV', 'V', 'I'), rockCtx);
    expect(numerals(apps[0].apply().slots!)).toBe('I IV V vi');
    const minorApps = SPICES['deceptive-cadence'].find(slotsOf('i', 'iv', 'V', 'i'), minorCtx);
    expect(numerals(minorApps[0].apply().slots!)).toBe('i iv V bVI');
  });

  it('chromatic mediant jumps a major third from home', () => {
    const apps = SPICES['chromatic-mediant'].find(slotsOf('I', 'V'), rockCtx);
    const bvi = apps.find((a) => a.label.includes('A♭'));
    expect(bvi).toBeDefined();
    expect(numerals(bvi!.apply().slots!)).toBe('I bVI V');
  });

  it('common-tone dim decorates a later tonic in major only', () => {
    const apps = SPICES['common-tone-dim'].find(slotsOf('I', 'IV', 'I'), rockCtx);
    expect(numerals(apps[0].apply().slots!)).toBe('I IV #iio7 I');
    expect(chordSymbol(resolveNumeral('#iio7', CMAJ))).toBe('D♯dim7');
    expect(SPICES['common-tone-dim'].find(slotsOf('i', 'iv', 'i'), minorCtx)).toHaveLength(0);
  });

  it('neapolitan leans onto V from a half step above home', () => {
    const apps = SPICES.neapolitan.find(slotsOf('i', 'iv', 'V'), minorCtx);
    expect(numerals(apps[0].apply().slots!)).toBe('i iv bII V');
  });

  it('deflated dominant lets the air out of V', () => {
    const apps = SPICES['minor-deflation'].find(slotsOf('I', 'V', 'vi', 'IV'), rockCtx);
    expect(numerals(apps[0].apply().slots!)).toBe('I v vi IV');
  });
});
