...
/**
 * Fit a chord onto a fixed keyboard window with ≤5 keys (one hand), preferring
 * root position, compact span and a centered hand. Shared by OP-1 and piano.
 */
export function windowChordVoicing(chord: Chord, baseMidi: number, keyCount: number): Op1Voicing {
  const tones = chord.quality.tones;
  const rootPc = notePc(chord.root);
  const TOP = baseMidi + keyCount - 1;
  const center = baseMidi + (keyCount - 1) / 2;
  ...
  for (const drop of attempts) {
    ...
    for (let k = 0; k < pcs.length; k++) {
      ...
      const score = span / 12 + Math.abs(mean - center) / 8 + (k > 0 ? 0.25 + 0.05 * k : 0);
      if (score < bestScore) {
        bestScore = score;
        best = {
          midis,
...
export const op1ChordVoicing = (chord: Chord): Op1Voicing =>
  windowChordVoicing(chord, OP1_BASE_MIDI, OP1_KEY_COUNT);

/** Key indices a scale lights up within a keyboard window; root keys flagged. */
export function windowScaleKeys(
  tonicPc: PitchClass, pcs: PitchClass[], baseMidi: number, keyCount: number,
): { index: number; isRoot: boolean }[] {
  const set = new Set(pcs.map(mod12));
  const out: { index: number; isRoot: boolean }[] = [];
  for (let i = 0; i < keyCount; i++) {
    const pc = mod12(baseMidi + i);
    if (set.has(pc)) out.push({ index: i, isRoot: pc === mod12(tonicPc) });
  }
  return out;
}

export const op1ScaleKeys = (tonicPc: PitchClass, pcs: PitchClass[]): { index: number; isRoot: boolean }[] =>
  windowScaleKeys(tonicPc, pcs, OP1_BASE_MIDI, OP1_KEY_COUNT);
