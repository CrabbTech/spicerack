...
export function scaleFretboard(tonicPc: PitchClass, pcs: PitchClass[], maxFret = MAX_FRET): FretboardNote[] {
...
/** The "position 1" box: a 5-fret window anchored on the low-E root fret. */
export function scaleBox(tonicPc: PitchClass, pcs: PitchClass[]): FretboardNote[] {
  let rootFret = mod12(tonicPc - OPEN_PC[0]);
  if (rootFret === 0) rootFret = 12;
  const lo = rootFret - 1;
  const hi = rootFret + 3;
  return scaleFretboard(tonicPc, pcs).filter((n) => n.fret >= lo && n.fret <= hi);
}

// ---------------------------------------------------------------------------
// ASCII tab.

export function chordTabText(entries: { symbol: string; voicing: GuitarVoicing }[]): string {
...
export function scaleTabText(name: string, box: FretboardNote[]): string {
