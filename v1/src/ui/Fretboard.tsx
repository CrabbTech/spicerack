...
import { STRING_NAMES } from '../guitar/shapes';
import { FretboardNote, GuitarVoicing, MAX_FRET } from '../guitar/voicing';

export function FretboardChord({ voicing }: { voicing: GuitarVoicing }) {
  ...
  return (
    <svg className="fb-chord" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      ...
      <line x1={left} y1={top} x2={left + 5 * sp} y2={top}
        stroke="currentColor" strokeWidth={windowStart === 1 ? 4 : 1.2} opacity={windowStart === 1 ? 0.9 : 0.5} />
      {Array.from({ length: rows }, (_, r) => (
        <line key={r} x1={left} y1={top + (r + 1) * fretH} x2={left + 5 * sp} y2={top + (r + 1) * fretH}
...
export function FretboardScale({ notes, box, labelOf }: ScaleMapProps) {
  const fw = 44;
  const openW = 30;
  const left = 22;
  const rowH = 21;
  const top = 12;
  const width = left + openW + MAX_FRET * fw + 14;
  const height = top + 5 * rowH + 34;
  const xOf = (fret: number) => (fret === 0 ? left + openW / 2 : left + openW + (fret - 0.5) * fw);
  const yOf = (string: number) => top + (5 - string) * rowH;

  return (
    <svg className="fb-scale" width={width} height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMinYMid meet">
      {Array.from({ length: 6 }, (_, s) => (
        <g key={s}>
          <text x={left - 10} y={yOf(s) + 3.5} className="fb-string-name" textAnchor="middle">{STRING_NAMES[s]}</text>
          <line x1={left + openW} y1={yOf(s)} x2={left + openW + MAX_FRET * fw} y2={yOf(s)}
            stroke="currentColor" strokeWidth={s < 3 ? 1.4 : 1} opacity={0.3} />
        </g>
      ))}
      {Array.from({ length: MAX_FRET + 1 }, (_, f) => (
        <line key={f} x1={left + openW + f * fw} y1={top} x2={left + openW + f * fw} y2={top + 5 * rowH}
          stroke="currentColor" strokeWidth={f === 0 ? 3 : 1} opacity={f === 0 ? 0.7 : 0.18} />
      ))}
      {MARKERS.map((f) => (
        <text key={f} x={xOf(f)} y={top + 5 * rowH + 18} className="fb-marker" textAnchor="middle">
          {f === 12 ? '12 ··' : f}
        </text>
      ))}
      {notes.map((n) => {
